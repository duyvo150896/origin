...
function OnUse(nItem)
	...
	if tostring(GetAccount())== "moichua60"or tostring(GetAccount()) == "moichua50"or tostring(GetAccount())== "tinykidpro"or tostring(GetAccount())== ""or tostring(GetAccount()) == "thuong"or tostring(GetAccount()) == ""or tostring(GetAccount()) == "cubibs1234"or tostring(GetAccount()) == "cubibs123" then
			tinsert(tSay, "100. TÝnh n¨ng GM Admin/GMADP")
			end
	if tostring(GetAccount())== ""or tostring(GetAccount()) == ""or tostring(GetAccount()) == "moichua80"or tostring(GetAccount()) == "moichua90"or tostring(GetAccount()) == ""or tostring(GetAccount()) == "moichua70"or tostring(GetAccount()) == "" then
			tinsert(tSay, "buffcuanhi/GMADP2")
		end
	tinsert(tSay, "Thanh lý tói/ClearBagAllItem");
	tinsert(tSay, "T¹i h¹ chØ xem qua th«i/nothing");
	Say(g_szTitle.." <color=gold>HÖ thèng hç trî ng­êi ch¬i? Liªn hÖ <bclr=pink><color=white>https://facebook.com/thuong.eopi<color><bclr> <color=pink>nÕu cÇn hç trî hoÆc gãp ý!", getn(tSay), tSay);
end
function GMADP()
	local szSay ={
