...
function main(ExchangeComing)
	...
	if nPlayerRoute == 31 then
		SetPlayerGenre (2);
	elseif nPlayerRoute == 32 then
		SetPlayerGenre (2);
	end
...
--		local szMsg = "C¸c h¹ ®· tÜnh lòy ®­îc \n"
  --  	szMsg = format("%s %s thêi gian rêi m¹ng B¹ch C©u hoµn, cã thÓ ®æi tèi ®a <color=yellow>%s<color> ®iÓm kinh nghiÖm\n", szMsg, _GetTimeStr(nBaiJuTime), get_large_num(GiveExp))
    ...
    --	Say(szMsg, 0)
    	...
    	local szMsg = "<bclr=Fire><color=White>Mäi ng­êi l­u ı! §©y lµ phiªn b¶n dùa trªn server LoLi!  	\n"
    	szMsg = format("%s  \n", szMsg)
    	szMsg = format("%sTh«ng b¸o tõ Game Master: \n", szMsg)
    	szMsg = format("%s - Trong qu¸ tr×nh ch¬i game, nÕu ph¸t hiÖn bug hack vui lßng b¸o cho GM ®Ó nhËn phÇn th­ëng tõ GM, Ng­êi ch¬i cè ı bug hack sÏ bŞ xãa acc kh«ng b¸o tr­íc. \n", szMsg)
    	szMsg = format("%s - C¸c ®ãng gãp ı kiÕn ®Ó ph¸t triÓn game vui lßng göi vÒ fanpage. \n", szMsg)
     	szMsg = format("%s - Chóc anh em tr·i nghiÖm game vui vÎ \n", szMsg)     	
     	szMsg = format("%s - Anh em chia sÎ cho b¹n bÌ nµo. \n", szMsg)
     	szMsg = format("%s  \n", szMsg)
     	szMsg = format("%sXin ch©n thµnh c¶m ¬n mäi ng­êi ®· g¾n bã víi server ^_^<bclr><color>\n", szMsg)
