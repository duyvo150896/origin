...
--		local szMsg = "C¸c h¹ ®· tÜnh lòy ®­îc \n"
  --  	szMsg = format("%s %s thêi gian rêi m¹ng B¹ch C©u hoµn, cã thÓ ®æi tèi ®a <color=yellow>%s<color> ®iÓm kinh nghiÖm\n", szMsg, _GetTimeStr(nBaiJuTime), get_large_num(GiveExp))
    ...
    --	Say(szMsg, 0)
    	...
    	local szMsg = "<bclr=Fire><color=White>Mäi ng­êi l­u ı! TÊt c¶ c¸c th«ng tin cËp nhËt míi nhÊt sÏ ®­îc ghi chó t¹i ®©y!  	\n"
    	szMsg = format("%s  \n", szMsg)
    	szMsg = format("%sDanh s¸ch c¸c cËp nhËt: \n", szMsg)
    	szMsg = format("%s - CËp nhËt tr¹ng th¸i tr¶i nghiÖm cña server. \n", szMsg)
    	szMsg = format("%s - Cho phĞp nhËn Tói hµng hiÖu trong BĞ LoLi. \n", szMsg)
     	szMsg = format("%s - Cho phĞp nhËn mËt tŞch VIP trong BĞ LoLi. \n", szMsg)
     	szMsg = format("%s - Cho phĞp nhËn Viªn h¾c tinh trong BĞ LoLi. \n", szMsg)
     	szMsg = format("%s - Anh em chia sÏ cho b¹n bÌ nµo. \n", szMsg)
     	szMsg = format("%s  \n", szMsg)
