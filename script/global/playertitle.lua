...
tDanhHieu = {
	-- [titleGD] = {{"msg", {"state_string", value, time_effect, 1}}},
	["_70_01_"] = {
		{"T¨ng giíi h¹n m¸u 40000", {"state_life_max_point_add", 40000, -1, -1}},
		{"Hç trî mËt tÞch t¨ng 15%", {"state_add_book_attribute_value", 15, -1, -1}},
		{"Kh¸ng tÊt c¶ +30", {"state_add_allability", 30, -1, -1}},
		{"Phßng thñ b¹o kÝch t¨ng 50", {"state_def_critical_point_increase", 50, -1, -1}},
		{"Gi¶m s¸t th­¬ng b¹o kÝch t¨ng 10", {"state_def_critical_damage_increase", 10, -1, -1}},
		{"Thêi gian hç trî tÊn c«ng t¨ng 10%", {"state_attack_buff_time", 10, -1, -1}},
		{"Thêi gian hç trî phßng thñ t¨ng 10%", {"state_defence_buff_time", 10, -1, -1}},
	}, -- ok
	["_70_02_"] = {
		{"T¨ng giíi h¹n m¸u 40000", {"state_life_max_point_add", 40000, -1, -1}},
		{"Hç trî mËt tÞch t¨ng 15%", {"state_add_book_attribute_value", 15, -1, -1}},
...
--hiÖu 71
	["_71_02_"] = {--hiÖu hµng hiÖu th­êng
		{"T¨ng giíi h¹n m¸u 40000", {"state_life_max_point_add", 40000, -1, -1}},
		{"Hç trî mËt tÞch t¨ng 15%", {"state_add_book_attribute_value", 15, -1, -1}},
		{"Kh¸ng tÊt c¶ +30", {"state_add_allability", 30, -1, -1}},
		{"Phßng thñ b¹o kÝch t¨ng 50", {"state_def_critical_point_increase", 50, -1, -1}},
		{"Gi¶m s¸t th­¬ng b¹o kÝch t¨ng 15", {"state_def_critical_damage_increase", 15, -1, -1}},
		{"Thêi gian hç trî tÊn c«ng t¨ng 10%", {"state_attack_buff_time", 10, -1, -1}},
		{"Thêi gian hç trî phßng thñ t¨ng 10%", {"state_defence_buff_time", 10, -1, -1}},
		{"Sinh lùc tèi ®a t¨ng 10%", {"state_life_max_percent_add", 10, -1, -1}},
		{"T¨ng thªm hiÖu qu¶ th­¬ng d­îc 4%", {"state_medicine_time_increase", 4, -1, -1}},
	}, -- ok

	["_71_03_"] = { --hiÖu hµng hiÖu VIP
		[0] = {
			{4,1,1},
			{5,0,1},
			{6,2,1},
			{8,6,1},
			{9,5,1} --dung hieu ung chua okay
		}; -- id hieu ung
		{"T¨ng giíi h¹n m¸u 40000", {"state_life_max_point_add", 40000, -1, -1}},
		{"Hç trî mËt tÞch t¨ng 15%", {"state_add_book_attribute_value", 15, -1, -1}},
		{"Kh¸ng tÊt c¶ +30", {"state_add_allability", 30, -1, -1}},
		{"Phßng thñ b¹o kÝch t¨ng 60", {"state_def_critical_point_increase", 60, -1, -1}},
		{"Gi¶m s¸t th­¬ng b¹o kÝch t¨ng 15", {"state_def_critical_damage_increase", 15, -1, -1}},
		{"Thêi gian hç trî tÊn c«ng t¨ng 10%", {"state_attack_buff_time", 10, -1, -1}},
		{"Thêi gian hç trî phßng thñ t¨ng 10%", {"state_defence_buff_time", 10, -1, -1}},
		{"Sinh lùc tèi ®a t¨ng 15%", {"state_life_max_percent_add", 15, -1, -1}},
		{"T¨ng thªm hiÖu qu¶ th­¬ng d­îc 6%", {"state_medicine_time_increase", 6, -1, -1}},
	}, -- ok
}

function _equipTitle(nGeren, nDetail, nType)
	local titleGD = format("_%.2d_%.2d_", nGeren, nDetail)
	local nStateID = nGeren * 10000 + nDetail * 100
	if nType == 2 then
		_unEquipTitle(nGeren, nDetail)
	else
		if not tDanhHieu[titleGD] then
			-- neu chua co trong he thong moi thi chay lai he thong cu
			local szfunc = format("_%.2d_%.2d_(%d, %d)", nGeren, nDetail, nType, nStateID)
			dostring(szfunc)
			--Msg2Player("Danh hiÖu nµy ch­a cã s½n")
		else
			local tCurTitle = tDanhHieu[titleGD]
			for i, v in tCurTitle do
				if tostring(i) ~= "n" and v ~= nil and type(v) == "table" and i > 0 then
					CastState(v[2][1], v[2][2], v[2][3], v[2][4], nStateID+i)
					Msg2Player("NhËn hiÖu qu¶: "..v[1])
					PlaySound("\\sound\\sound_i016.wav");
				end
			end
			if tCurTitle[0] ~= nil then
				for i=1, getn(tCurTitle[0]) do --bem nha fail c¸i b¶n npc bro tim ë ®©u ra
					SetCurrentNpcSFX(PIdx2NpcIdx(), tCurTitle[0][i][1], tCurTitle[0][i][2], tCurTitle[0][i][3]); -- xong r do uin no1 ko tha1o
				end;
			end;
		end
	end
...
function _unEquipTitle(nGeren, nDetail)
	local titleGD = format("_%.2d_%.2d_", nGeren, nDetail)
	local nStateID = nGeren * 10000 + nDetail * 100
	if not tDanhHieu[titleGD] then
		-- Do nothing
		...
		for i, v in tCurTitle do
			if tostring(i) ~= "n" and v ~= nil and type(v) == "table" and i > 0 then
				RemoveState(nStateID+i)
				Msg2Player("Hñy hiÖu qu¶: "..v[1])
			end
		end
		if tCurTitle[0] ~= nil then
			for i=1, getn(tCurTitle[0]) do
				SetCurrentNpcSFX(PIdx2NpcIdx(), 0, tCurTitle[0][i][2], tCurTitle[0][i][3]); -- xong r do uin no1 ko tha1o
			end;
		end;
	end
end
...
function Equip(nGeren, nDetail, nType)
	...
	local szfunc = format("_%.2d_%.2d_(%d, %d)", nGeren, nDetail, nType, nStateID)
	--dostring(szfunc)
	_equipTitle(nGeren, nDetail, nType)
	if nType == 1 then
...
function _71_01_(nType, StateID)--buff
	if nType == 1 then
		CastState("state_life_max_point_add",40000, -1, 1, StateID);
		SetCurrentNpcSFX(PIdx2NpcIdx(),4,1,1);
