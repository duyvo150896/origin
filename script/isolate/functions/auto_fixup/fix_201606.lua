...
function t_Fix_201606:fix_up(nLastLoginDate)
	if nLastLoginDate <= self.nTimeLine then
		...
		if self:is_in_black_list(szAccount, szRole)==1 then
    		...
    		local szMsg = format("§¹i hiÖp do sö dông ph­¬ng thøc kh«ng tÝch ®¸ng ®Ó nhËn phÇn th­ëng Vinh Danh B©ng Héi, thêi gian B¹ch CÇu, thêi gian Lôc ThÇn, thêi gian Tan Thanh vµ ®iÓm kinh nghiÖm c¶u ®¹i hiÖp ®· bÞ thiÕt lËp l¹i. NÕu cã ý kiÕn hoÆc vÊn ®Ò kh¸c mêi ®¹i hiÖp liªn l¹c víi hç trî game.")
    		Msg2Player(szMsg)
    		Talk(1,"",szMsg)
    	end
	end
