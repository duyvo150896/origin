...
DBMC_NPC_NAME = "<color=green>Binh Bé ThŞ Lang<color>:"
...
g_Dbmc_tTask = {
	...
	{"Cæ D­¬ng ®éng_2", "Xİch LuyÖn Háa Hå", 20, 512},
	--{"´ó²İÔ­¶ş", "Òì×å¼ıÊÖ", 20, 602},
	{"H¾c Phong ®éng", "M·ng KhÊu", 20, 215},
	--{"Á½Ë®¶´¶ş²ã", "ò¿»ğ½ÌÍ½", 20, 113},
	--{"¹íÃÅ»ØÀÈ", "Ğ¡À¶µ¶¹í", 20, 328},
}

g_Dbmc_tAward = {
	{"item", 10, "Da §Şa Long", {2, 1, 30673, 100}},
...
function on_init_ok()
	...
	AcivityBase.TaskGroup.Task2 = 2; --µ±Ç°ÈÎÎñË÷Òı1£¬É±¹Ö´ÎÊı2£¬Ã¿ÈÕ½ÓÈ¡ÈÎÎñ¸öÊı3, µÁ±¦Ôô¼ÆÊı4
	AcivityBase.TaskGroup.Task3 = 3; --ÆÕÍ¨²Ø±¦Í¼Ê¹ÓÃ´ÎÊı
	AcivityBase.TaskGroup.Task4 = 4; --×ø±êË÷Òı
	AcivityBase.TaskGroup.Task5 = 5; --µÁÄ¹ÔôµÄ²Æ±¦Ê¹ÓÃ´ÎÊı
	AcivityBase.TaskGroup.Task6 = 6; --µÁÄ¹ÔôµÄ²Æ±¦ÀÛ¼Æ½±Àø±ê¼Ç
	AcivityBase.TaskGroup.Task7 = 7; --Ë¢ĞÂÉ±¹Ö¿ª¹Ø±ê¼Ç1,2,3bit
	AcivityBase.TaskGroup.Task8 = 8; --¼ÇÂ¼Ë¢¹ıµÁÄ¹ÔôµÄ²Æ±¦
	isolate_enable_event_dispatch(-1)
end

function on_add_watches()
	isolate_watch_npc_talk("Binh Bé ThŞ Lang", 200, "handle_npc_talk_main")
...
function handle_npc_talk_main(nNpcIndex)
	...
	local msg = "GÇn ®©y Binh Bé ThŞ Lang tiªu diÖt mét nhãm §¹o Mé TÆc trong L¨ng TÇn Thñy Hoµng, nhËn ®­îc rÊt nhiÒu Tµng B¶o §å. Bçng nhiªn khiÕn cho nh÷ng ng­¬i cã t©m ®Şa xÊu ®ét nhËp vµo ThŞ Lang Phñ khiÕn cho Binh Bé ThŞ Lang phiÒn lßng; ®Ó gi¶i quyÕt chuyÖn nµy th× Binh Bé ThŞ Lang ®· ban th­ëng Tµng B¶o §å ®Ó triÖu tËp hiÖp sÜ giang hå ®Ó trõng trŞ chóng."
	local tSay = {
		"NhiÖm vô Trõng Giíi/reprimand_task",
		"PhÇn th­ëng tİch lòy Tµi S¶n §¹o B¶o TÆc/treasure_accumulative_award",
...
function reprimand_task()
	...
	isolate_enable_player_event_watch("event_kill_npc", "kill_npc_task_on_trigger"); --ÉúĞ§
...
function event_treasure_deal(nType)
	...
	elseif nType == 3 then
		...
		isolate_enable_player_event_watch("event_kill_npc", "kill_npc_baozangshouhuzhe")
		Talk(1,"","Lì tay kİch ho¹t c¬ quan, bŞ ng­êi b¶o vÖ ph¸t hiÖn!")
	elseif nType == 4 then
...
function treasure_accumulative_award()
	...
	msg = msg.."(sau khi nhËn xong phÇn th­ëng tİch lòy mét l­ît sÏ tù ®éng reset)"
	local tSay = {}
...
function treasure_accumulative_award_cb(nIndex)
	...
	if tAward[1] == "item" then
		gf_AddItemEx2(tAward[4], tAward[3], "§¹o B¶o Mª Thµnh", "Th­ëng tİch lòy Tµi S¶n §¹o B¶o TÆc", 0, 1);
	else
	...
	if bflag ~= 0 then
		...
		Msg2Player("§· reset phÇn th­ëng tİch lòy");
	end
end