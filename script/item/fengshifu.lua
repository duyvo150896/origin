...
USE_FENGSHIFU_INTERVAL = 18;
TASK_USE_FSF_DATE = 2060;
TASK_USE_FSF_TIME = 2061;
...
--Èç¹û½ñÌì»¹Ã»ÓÐÖÖÊ÷£¬Ôò±íÊ¾¿ÉÒÔÖÖÊ÷
	if GetTask(TASK_USE_FSF_DATE) < nDate then 
		SetTask(TASK_USE_FSF_TIME, 0);
		SetTask(TASK_USE_FSF_DATE, nDate);
	else

	
	if GetTime() - GetTask(TASK_USE_FSF_TIME) < USE_FENGSHIFU_INTERVAL then
		Talk(1, "", "Thêi gian gi·n c¸ch gi÷a hai lÇn sö dông lµ <color=yellow>" .. USE_FENGSHIFU_INTERVAL .. " gi©y.");
		return 0;
	end
	...
	SetTask(TASK_USE_FSF_TIME, GetTime());
