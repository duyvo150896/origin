...
--		"Ta muèn ®æi Kim xµ Khëi Ph­îng (chñ yÕu néi c«ng)/#Select_TypeChildKimXa(34)",
	...
	Say("Dïng <color=gold>Ph¸ Hån LÖnh (QuÇn)<color> ®Ó cã thÓ hîp thµnh <color=gold>Ph¸ Hån H¹ Y,<color>dïng ngay?", getn(tSay), tSay);
end
...
function SelectOption(nType, nTypeChild, OptionIndex, OptionValue)
	...
	if (OptionIndex + 1) < 4 then
		...
		Say("Chän thuéc tÝnh <color=gold>thø "..(OptionIndex + 1).."<color> cho trang bÞ <color=gold>Ph¸ Hån H¹ Y<color>:", getn(tSay), tSay);
	else 
...
function Ask(nType, nTypeChild)
	...
	tSayTitle = tSayTitle.."QuyÕt ®Þnh dïng <color=gold>Ph¸ Hån LÖnh (QuÇn)<color> ®æi thµnh <color=gold>Ph¸ Hån H¹ Y<color>?\n";
	
	tinsert(tSay, "§ång ý ®æi/#Get_Item("..nType..","..nTypeChild..")")
	tinsert(tSay, "\n§Ó ta suy nghÜ l¹i/nothing")
	
	Say("Thuéc tÝnh lùa chän cña <color=gold>Ph¸ Hån H¹ Y<color> nh­ sau:\n"..tSayTitle, getn(tSay), tSay);
end
...
function Get_Item(nType, nTypeChild)
	if DelItemByIndex(nItem, 1) == 1 then
		...
		SetItemFeedUpAttrs(pIndex,3683,0,0,OptionTable[1],OptionTable[2],OptionTable[3])
		end
	...
	--	SetItemFeedUpAttrs(pIndex,3683,0,0,OptionTable[1],OptionTable[2],OptionTable[3])
		...
		Msg2Player("B¹n nhËn ®­îc 1 Ph¸ Hån H¹ Y")
	end
end
