...
---Fix by thanhduongjokoe from clbgameviet.net
--»î¶¯²ú³öÍ·ÎÄ¼þ°üº¬
...
function ttd_DailyWeeklyTask(nType)
	if nType == 1 then
		if tGtTask:check_cur_task(83) == 1 and GetTask(VET_TDD_DALIY_TASK_CROSS) ~= 3 then
			SetTask(VET_TDD_DALIY_TASK_CROSS,1);
			Msg2Player(format("NhiÖm vô ngµy - th«ng quan Th«ng Thiªn HuyÔn C¶nh%d/3 lÇn (hoµn thµnh) ",GetTask(VET_TDD_DALIY_TASK_CROSS)));
		end
		if tGtTask:check_cur_task(84) == 1 and GetTask(VET_TDD_WEEK_TASK_CROSS) < 10 then
			SetTask(VET_TDD_WEEK_TASK_CROSS,GetTask(VET_TDD_WEEK_TASK_CROSS) + 1);
			if GetTask(VET_TDD_WEEK_TASK_CROSS) == 10 then
				Msg2Player(format("NhiÖm vô tuÇn - th«ng quan Th«ng Thiªn HuyÔn C¶nh%d/10 lÇn (hoµn thµnh) ",GetTask(VET_TDD_WEEK_TASK_CROSS)));
			else
				Msg2Player(format("NhiÖm vô tuÇn - th«ng quan Th«ng Thiªn HuyÔn C¶nh%d/10 lÇn ",GetTask(VET_TDD_WEEK_TASK_CROSS)));
			end
		end
	elseif nType == 2 then
