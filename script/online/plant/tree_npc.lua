...
function givewater()
	...
	if nstate ~= 2 and nstate ~= 3 then
		Talk(1,"","Yªu cÇu tr¹ng th¸i ®å s¸t c¸ nh©n hoÆc ®å sat bang héi ®Ó t­íi c©y");
		return 0;
	end
	----
...
function getfruit()
	...
	local nstate, npkcamp = GetPKFlag()
	if nstate ~= 2 and nstate ~= 3 then
		Talk(1,"","Yªu cÇu tr¹ng th¸i ®å s¸t c¸ nh©n hoÆc ®å sat bang héi ®Ó thu hoach c©y");
		return 0;
	end
	----
	if GetTask(TASK_TREEGROW) == 6 then
		...
		-- if nRandomNum <= 100 then
			ModifyExp(nRandomexp);	--¶þÊ®Íò¾­Ñé
			Msg2Player("B¹n nhËn ®­îc 1 ®èng v¹n kinh nghiÖm");
			-- WriteLog("[Ho¹t ®éng trång c©y Th¸i H­]:"..GetName().."NhËn ®­îc 20 v¹n kinh nghiÖm");
		-- end;
		-- if GetLevel() < 90 then
			-- if nRandomNum <= 100 then
				-- ModifyExp(5000000);	--¶þÊ®Íò¾­Ñé
				-- Msg2Player("B¹n nhËn ®­îc 500 v¹n kinh nghiÖm");
				-- WriteLog("[Ho¹t ®éng trång c©y Th¸i H­]:"..GetName().."NhËn ®­îc 500 v¹n kinh nghiÖm");
			-- end;
		-- end
		local nDate_1 =  tonumber(date("%H"))
		-- if GetLevel() < 98 and GetLevel() >= 90 then
			-- ModifyExp(10000000);
			-- Msg2Player("B¹n nhËn ®­îc 10000000 kinh nghiÖm");
		...
		-- if GetLevel() == 99 then
			-- ModifyExp(1000000);
			-- Msg2Player("B¹n nhËn ®­îc 1000000 kinh nghiÖm");
			-- if nDate_1 == 18 or nDate_1 == 22 then
				-- ModifyExp(1000000);
				-- Msg2Player("B¹n nhËn ®­îc 1000000 kinh nghiÖm");
		...
		-- if nDate_1 == 18 or nDate_1 == 22 then
			-- AwardGenuineQi(100);
			-- Msg2Player(format("C¸c h¹ nhËn ®­îc %d ®iÓm ch©n khÝ", 100))
		-- end
		...
		if nRandomNum <= 50 then
			if AddItem(1,0,32,10) == 1 then
				...
				WriteLog("[Ho¹t ®éng trång c©y Th¸i H­]:"..GetName().."NhËn ®­îc 10 Cöu ChuyÓn Hoµn Hån §¬n");
			end;		
		end;
