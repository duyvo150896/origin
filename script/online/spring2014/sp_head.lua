...
SP_TASK_GROUP = TaskManager:Create(7, 10);
SP_TASK_GROUP.DaySeq = 1;
SP_TASK_GROUP.DayAcc = 2; --ÀÛ¼Æ°ÝÁË¼¸´Î
SP_TASK_GROUP.TargetNpc = 3; --¼Ç¢¼°ÝÄênpcµÄTableIndex
SP_TASK_GROUP.IsAward = 4;  --ÊÇ·ñÁì½±
SP_TASK_GROUP.MaxUse = 5; --Ã¿Ìì×î¶µÊ¹ÓÃ
SP_TASK_GROUP.UseTimes = 6; --´òÄêÊÞÌìÊý
SP_TASK_GROUP.DropBox = 7; --ÊÇ·ñµô¢ä±¦Ïä
SP_TASK_GROUP.ThsDrop = 8; --ÌÒ»¨Ê÷µô¢ä
...
function sp_CreateDecorate()
	...
	--ÏåÑô
	if SubWorldID2Idx(350) ~= nil then
		--12
		do
			npcIndex = CreateNpc("Phi YÕn Tiªn Tö","H»ng Nga Tiªn Tö",  100, 1410, 2960)
			-- SetCurrentNpcSFX(PIdx2NpcIdx(),978,1,1)
			-- SetNpcScript(npcIndex, "\\script\\online\\spring2014\\sp_npc_ths_2.lua");
		end
		do
			local npcIndex = CreateNpc("B¶o r­¬ng Tµi nguyªn"," R­¬ng May M¾n ", 100, 1416,2960);
			SetNpcScript(npcIndex, "\\script\\online\\spring2014\\sp_npc_ths_1.lua");
		end
