...
-- TASK NEW SERVER
if nDate >= 161029 and nDate <= 161210 then
...
TASK_TICH_LUY_DIEM_THUONG = 2430
...
------------------
TSK_GIAPNGO_1 = 2675	--event ®µo r­¬ng gi¸p ngä: H: nhËn th­ëng kİch ho¹t CDPB, gggg: sè lÇn kİch ho¹t ChiÕu D¹ Ph¸p B¶o, Y: nhËn th­ëng kİch ho¹t TABB, xxxx: sè lÇn kİch ho¹t TuyÖt Anh Bİ B¶o
...
-- Reset task më réng event th¸ng 10
	...
	if tonumber(date("%y%m%d%H")) >= 16100100 then
	...
	if tonumber(date("%y%m%d%H")) >= 16090500 then
	...
	if tonumber(date("%y%m%d%H")) >= 16110400 then
...
function VietResetDate()
	local TASKID_TY_NORMAL_TIMES_LIMIT		= 3466;
	local TASKID_TY_HERO_TIMES_LIMIT		= 3467;
	local nDate = tonumber(date("%y%m%d"))
...
--		SetTask(TSK_LUCKY_DAOHUU_NEW_THANG4,0)	
		...
		SetTask(TASKID_TY_NORMAL_TIMES_LIMIT,0)
		SetTask(TASKID_TY_HERO_TIMES_LIMIT,0)
		SetTask(TASKID_LSB_TIMES_LIMIT,0)
...
--		KF_Daily_Reset() --reset tham gia chiÕn tr­êng liªn server
		...
		if nDate <= 161210 then	-- Dua top cong hien bang server Vo Dang Son + Duong Gia Tuong
