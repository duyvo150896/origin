...
--	end
	...
	if nDate < 20130904 then
		tinsert(tbSay,"NhËn h?tr?trang b?Kim X?_On_Repair")
	end
--	tinsert(tbSay, "Ta muèn ®æi nguyªn liÖu lÊy kinh nghiÖm/Trade_Material2Exp_Menu")
	...
	-- tinsert(tbSay,"§æi trang b?DiÖu D­¬ng/NangCapDieuDuong")
...
--	tinsert(tbSay, "§æi trang b?ChiÕn Cuång/TrangBiChienCuong_Form")
...
function pay_award(nNum1,nNum2,nNum3)
	...
	if nNum2 == 0 then
		...
		AskClientForNumber("pay_wooden",1,nNum1,"H·y nhËp s?l­îng","")
	end
	if nNum2 == 1 then
		if nNum < nNum1 * nNum3 then nNum1 = floor(nNum / nNum3) end
		AskClientForNumber("pay_treasure",1,nNum1,"H·y nhËp s?l­îng","")
	end
end
...
function get_BKVD()
	local tbSayDialog = {};
	local nSaySize = 0;
	local szSayHead = "H·y chän trang b?cÇn n©ng cÊp."
	
	tinsert(tbSayDialog, "- Ta cÇn ®æi 1 B¹ch Kim Viªm §Õ Kh«i (100 M¶nh B¹ch Kim + 3 B¹ch Kim Hång Bao (c?chøa 4000 vµng + 1 Thiªn Chi Viªm §Õ Kh«i[+12])/#get_BKVD_confirm(1,0)")
	tinsert(tbSayDialog, "- Ta cÇn ®æi 1 B¹ch Kim Viªm §Õ Gi¸p (100 M¶nh B¹ch Kim + 3 B¹ch Kim Hång Bao (c?chøa 4000 vµng + 1 Thiªn Chi Viªm §Õ Gi¸p[+12])/#get_BKVD_confirm(2,0)")
	tinsert(tbSayDialog, "- Ta cÇn ®æi 1 B¹ch Kim Viªm §Õ Trang (100 M¶nh B¹ch Kim + 3 B¹ch Kim Hång Bao (c?chøa 4000 vµng + 1 Thiªn Chi Viªm §Õ Trang[+12])/#get_BKVD_confirm(3,0)")
...
function get_BKVD_confirm(nGetType, nSelected)
	if gf_CheckPlayerRoute() == 0 then
		Talk(1, "","Ph¶i gia nhËp h?ph¸i míi n©ng cÊp trang b?®­îc!");
		return 0;
	end
	...
	if GetItemCount(2,1,30346) < nItemNum1 then
		Talk(1, "", "B¹n kh«ng ®ñ "..tItem_need[1][1].." ®Ó ®æi trang b?B¹ch Kim Viªm §Õ")
		return 0
	end
	    	local nItemNum2 = tItem_need[2][3]
	if GetItemCount(2,1,30229) < nItemNum2 then
		Talk(1, "", "B¹n kh«ng ®ñ "..tItem_need[2][1].." ®Ó ®æi trang b?B¹ch Kim Viªm §Õ")
		return 0
	end
	SetTaskTemp(TSK_TMP_CHANGE_BKVD,nGetType)
	local nSlot = BKVD_need[nGetType][1]		
	PutinItemBox("§æi trang b? ,nSlot ", "X¸c nhËn muèn thùc hiÖn n©ng cÊp trang b?", FILE_NAME, 1)
end
...
function OnPutinCheck(param, idx, genre, detail, particular)
	...
	for i=1,getn(tItem) do
		...
		if (GetEquipAttr(idx,2) < 12) and genre == 0 then
			Talk(1,"",g_szNpcName.."§é c­êng hãa vËt phÈm ph¶i t?12 tr?lªn");
			return 0;		
		end
	...
	Talk(1, "", g_szNpcName.."Ch?c?th?b?vµo "..BKVD_need[nGetType][2]);
	return 0;
end
...
function OnPutinComplete(param)
	local nGetType = GetTaskTemp(TSK_TMP_CHANGE_BKVD)
	if nGetType == 0 then
		Talk(1, "", g_szNpcName.."B?vµo vËt phÈm kh«ng ®óng");
		return 0;
	end
	if param ~= 1 then
		Talk(1, "", g_szNpcName.."B?vµo vËt phÈm kh«ng ®óng");
	...
	if nResult ==1 then
		if DelItem(2,1,30346, nItemNum1) == 1 and DelItem(2,1,30229, nItemNum2) == 1 then
			...
			gf_AddItemEx2(nItem, "N©ng cÊp trang b?thµnh c«ng", "NANG CAP TRANG BI BKVD", "n©ng cÊp thµnh c«ng")
			Talk(1,"","Chóc mõng b¹n nhËn ®­îc 1 trang b?B¹ch Kim Viªm §Õ!")
		end
	end
end
...
function get_haonguyet_confirm(nGetType, nSelected)
	...
	if gf_Judge_Room_Weight(2, 500, "") == 0 then
     ...
     if nGetType == 1 then
			...
			Talk(1, "", "B¹n kh«ng ®ñ 8 m¶nh B¹ch Kim ®Ó ®æi trang b?H¹o NguyÖt")
			return 0
		end
     ...
     if nGetType == 2 then
			...
			Talk(1, "", "B¹n kh«ng ®ñ 6 m¶nh B¹ch Kim ®Ó ®æi trang b?H¹o NguyÖt")
			return 0
		end
    		if GetItemCount(2,1,30229) < 1 then
			Talk(1, "", "B¹n kh«ng ®ñ 1 B¹ch Kim §¹i Hång Bao ®Ó ®æi trang b?H¹o NguyÖt")
     ...
     if nGetType == 4 then
			...
			Talk(1, "", "B¹n kh«ng ®ñ 12 m¶nh B¹ch Kim ®Ó ®æi trang b?H¹o NguyÖt")
			return 0
		end    	
     ...
     if nGetType == 5 then
			...
			Talk(1, "", "B¹n kh«ng ®ñ 16 m¶nh B¹ch Kim ®Ó ®æi trang b?H¹o NguyÖt")
			return 0
		end    	
...
function Confirm_get_BKR(nCount)
	...
	if DelItem(2,1,30341, 5 * nCount) == 1 then
        ...
        gf_WriteLogEx("Than Tai Bao Ruong", "®æi thµnh c«ng", nCount, "S?l­îng B¹ch Kim R­¬ng")			
	end
end
...
function Confirm_get_TTBR(nCount)
	...
	if DelItem(2,1,30341, 55 * nCount) == 1 then
		...
		gf_WriteLogEx("Than Tai Bao Ruong", "®æi thµnh c«ng", nCount, "S?l­îng ThÇn Tµi R­¬ng")	
	end
end
...
function NangCapDieuDuong()
	local tbSay = {
		"N©ng cÊp b»ng Thiªn M«n Kim LÖnh v?trang b?H¹o NguyÖt/NangCapDieuDuong_CongThuc1_Form",
		"N©ng cÊp b»ng Thiªn M«n Kim LÖnh, C?Linh Ngäc v?trang b?H¹o NguyÖt/NangCapDieuDuong_CongThuc2_Form",
	}
	Say("N©ng cÊp trang b?H¹o NguyÖt thµnh DiÖu D­¬ng", getn(tbSay), tbSay)
end


function NangCapDieuDuong_CongThuc1_Form()
	local szTitle = "N©ng cÊp trang b?H¹o NguyÖt thµnh DiÖu D­¬ng cÇn:\n- Nãn: 5 Thiªn M«n Kim LÖnh\n- ¸o: 6 Thiªn M«n Kim LÖnh\n- QuÇn: 7 Thiªn M«n Kim LÖnh\n- Trang søc: 6 Thiªn M«n Kim LÖnh\n- V?Kh? 11 Thiªn M«n Kim LÖnh"
	local tbSay = {
...
function NangCapDieuDuong_CongThuc2_Form()
	local szTitle = "N©ng cÊp trang b?H¹o NguyÖt thµnh DiÖu D­¬ng cÇn:\n- Nãn: 3 Thiªn M«n Kim LÖnh, 500 C?Linh Ngäc\n- ¸o: 3 Thiªn M«n Kim LÖnh, 500 C?Linh Ngäc\n- QuÇn: 3 Thiªn M«n Kim LÖnh, 500 C?Linh Ngäc\n- Trang søc: 3 Thiªn M«n Kim LÖnh, 500 C?Linh Ngäc\n- V?Kh? 6 Thiªn M«n Kim LÖnh, 1000 C?Linh Ngäc"
	local tbSay = {
...
function NangCapDieuDuong_CongThuc1()
	PutinItemBox("N©ng cÊp trang b? ",1 , "X¸c nhËn muèn thùc hiÖn n©ng cÊp?", NANGCAPDIEUDUONG_F1, 1)
end


function NangCapDieuDuong_CongThuc2()
	PutinItemBox("N©ng cÊp trang b?" ,1 , "X¸c nhËn muèn thùc hiÖn n©ng cÊp?", NANGCAPDIEUDUONG_F2, 1)
end


function NangCapDieuDuong_NgocBoi()
	PutinItemBox("N©ng cÊp trang b?" ,1 , "X¸c nhËn muèn thùc hiÖn n©ng cÊp?", NANGCAPDIEUDUONG_NGOCBOI, 1)
end